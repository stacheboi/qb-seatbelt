        ...
        if IsControlJustReleased(0, 47) then 
            if not harnessOn then
                if IsPedInAnyVehicle(ped) and GetVehicleClass(GetVehiclePedIsIn(ped)) ~= 8 and GetVehicleClass(GetVehiclePedIsIn(ped)) ~= 13 and GetVehicleClass(GetVehiclePedIsIn(ped)) ~= 14 then
                    if seatbeltOn then
                        TriggerEvent('QBCore:Notify', 'Seat belt unbuckled', 'error', 1500)
                        TriggerEvent("qb-sound:client:play", "car-unbuckle", 0.25)
                        
                    else
                        TriggerEvent('QBCore:Notify', 'Seat belt buckled')
                        TriggerEvent("qb-sound:client:play", "car-buckle", 0.25)
                    end
